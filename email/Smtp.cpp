...
bool CSmtp::CreateConn()
{
	...
	if (this->isUseSSL) {
		...
		ssl = SSL_new(ctx);
		SSL_set_fd(ssl, this->sockClient);
		/* 建立 SSL 连接 */
...
{
	if (this->isUseSSL == 1) {
		int len = SSL_write(this->ssl, (void*)message.c_str(), message.length());
		if (len > 0) {}
		else {
			return false;
		}
...
bool CSmtp::Recv()
{
	...
	if (this->isUseSSL == 1) {
		...
		err = recv(sockClient, buff, MAXLEN, 0); //接收数据
		if (err == SOCKET_ERROR)
		{
			return false;
		}
